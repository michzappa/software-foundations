...
Lemma map_app : forall (X Y : Type) (f : X -> Y) (l : list X) (e : X),
...
Fixpoint filter_explicit (X:Type) (test: X->bool) (l:list X) : list X :=
  match l with
  | [] => []
  | h :: t =>
    if test h then h :: (filter_explicit X test t)
    else filter_explicit X test t
  end.

Fixpoint map_explicit (X Y : Type) (f : X->Y) (l : list X) : list Y :=
  match l with
  | []     => []
...
(* take in a string, return a map of <character, count> *)
